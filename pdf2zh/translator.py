...
try:
    ...
    logger.warning(
        "transformers, torch, or sentencepiece not installed. NLLB translator will not be available."
    )
...
class NLLBModelInstance:
    """Глобальный или лениво загружаемый экземпляр модели NLLB"""
    model = None
    tokenizer = None
    model_name = None
    device = None
    loading = False
    load_lock = threading.Lock() # Для потокобезопасной загрузки

    @classmethod
    def get_model_and_tokenizer(cls, model_name_req: str, device_req: str):
        with cls.load_lock:
            if cls.model is not None and cls.tokenizer is not None and cls.model_name == model_name_req and cls.device == device_req:
                # Модель уже загружена с нужными параметрами
                return cls.model, cls.tokenizer

            if cls.loading:
                # Другой поток уже загружает, ждем (или можно добавить более сложную логику ожидания)
                logger.warning("NLLB model is already loading in another thread. Waiting...")
                # Простая блокировка до завершения загрузки другим потоком
                # В реальном приложении лучше использовать Event или Condition
                while cls.loading:
                    import time
                    time.sleep(1)
                # После ожидания, проверяем снова, т.к. параметры могли измениться
                if cls.model is not None and cls.tokenizer is not None and cls.model_name == model_name_req and cls.device == device_req:
                    return cls.model, cls.tokenizer
                else: # Если параметры не совпали, придется перезагружать (или выдать ошибку)
                     logger.error("NLLB loading parameters conflict or failed.")
                     # Здесь можно либо снова инициировать загрузку, либо вернуть None/ошибку
                     # Пока просто выходим
                     return None, None
            ...
            # Начинаем загрузку
            cls.loading = True
            cls.model_name = model_name_req
            cls.device = device_req
            try:
                logger.info(f"Loading NLLB tokenizer: {cls.model_name}")
                cls.tokenizer = AutoTokenizer.from_pretrained(cls.model_name)
                logger.info(f"Loading NLLB model: {cls.model_name} onto device: {cls.device} (this may take a while)...")

                if cls.device == "auto":
                     # accelerate должен быть установлен
                    cls.model = AutoModelForSeq2SeqLM.from_pretrained(cls.model_name, device_map="auto")
                    # device_map сам выбирает устройство, model.device покажет реальное
                    logger.info(f"NLLB model loaded using device_map='auto'. Main device: {cls.model.device}")
                else:
                    actual_device = torch.device(cls.device if torch.cuda.is_available() and cls.device=="cuda" else "cpu")
                    cls.model = AutoModelForSeq2SeqLM.from_pretrained(cls.model_name).to(actual_device)
                    cls.device = str(actual_device) # Обновляем реальное устройство
                    logger.info(f"NLLB model loaded onto device: {cls.device}")

                return cls.model, cls.tokenizer
            except Exception as e:
                logger.error(f"Failed to load NLLB model '{cls.model_name}': {e}", exc_info=True)
                cls.model = None
                cls.tokenizer = None
                cls.model_name = None
                cls.device = None
                raise # Перевыбрасываем исключение, чтобы сообщить об ошибке
            finally:
                cls.loading = False # Завершаем загрузку (успешно или нет)
...
if NLLB_AVAILABLE:
    class NLLBTranslator(BaseTranslator):
        name = "nllb"
        envs = {
            # Модель по умолчанию (можно изменить через config.json или переменные окружения)
            "NLLB_MODEL_NAME": "facebook/nllb-200-distilled-600M",
            # Устройство: "auto" (требует accelerate), "cuda", "cpu"
            "NLLB_DEVICE": "auto",
        }
        # Карта для преобразования коротких кодов (из GUI) в коды FLORES-200
        internal_lang_map = {
            "en": "eng_Latn",
            "zh": "zho_Hans", # Упрощенный китайский
            "zh-TW": "zho_Hant", # Традиционный китайский
            "fr": "fra_Latn",
            "de": "deu_Latn",
            "ja": "jpn_Jpan",
            "ko": "kor_Hang",
            ...
            "es": "spa_Latn",
            "it": "ita_Latn",
            "mn": "khk_Cyrl", # Монгольский (кириллица) - проверьте, если нужен латинский khk_Latn
            # Добавьте другие языки по мере необходимости
        }
        CustomPrompt = False # NLLB обычно не требует сложных промптов для простого перевода

        def __init__(
            self, lang_in, lang_out, model_name_override=None, envs=None, ignore_cache=False, **kwargs
        ):
            self.set_envs(envs) # Загружаем envs из config/os.environ

            # Определяем имя модели и устройство
            model_name = model_name_override or self.envs["NLLB_MODEL_NAME"]
            device = self.envs["NLLB_DEVICE"]

            # Преобразуем короткие коды в коды FLORES-200
            self.nllb_lang_in = self.internal_lang_map.get(lang_in, lang_in) # Попытка маппинга
            self.nllb_lang_out = self.internal_lang_map.get(lang_out, lang_out) # Попытка маппинга

            # Важно: Инициализация BaseTranslator должна идти *после* определения nllb_lang_in/out,
            # если они влияют на ключ кэша, но NLLB использует model_name в ключе кэша.
            super().__init__(lang_in, lang_out, model_name, ignore_cache)

            # Ленивая загрузка модели при первом вызове do_translate
            self.model = None
            self.tokenizer = None
            self.required_model_name = model_name
            self.required_device = device

            # Добавляем параметры, влияющие на кэш
            self.add_cache_impact_parameters("nllb_model_name", self.required_model_name)
            self.add_cache_impact_parameters("nllb_device", self.required_device) # Хотя устройство не меняет результат, но может быть полезно для отладки
        ...
        def _ensure_model_loaded(self):
             """Загружает модель и токенизатор, если они еще не загружены"""
             if self.model is None or self.tokenizer is None:
                  self.model, self.tokenizer = NLLBModelInstance.get_model_and_tokenizer(
                       self.required_model_name, self.required_device
                  )
                  if self.model is None or self.tokenizer is None:
                       raise RuntimeError("Failed to load NLLB model or tokenizer.")


        # Можно добавить @retry декоратор, как в OpenAITranslator, если нужно
        def do_translate(self, text: str) -> str:
            self._ensure_model_loaded() # Убеждаемся, что модель загружена
            ...
            try:
                ...
                return translation.strip()
                 ...
                 # Возвращаем ошибку или пустую строку, чтобы не прерывать весь процесс
                 return f"[NLLB Translation Error: {e}]"

else:
     # Можно определить заглушку, если transformers не установлен
     class NLLBTranslator(BaseTranslator):
          name = "nllb"
          envs = {}
          def __init__(self, *args, **kwargs):
                raise ImportError("NLLB dependencies (transformers, torch, sentencepiece, accelerate) are not installed.")
          def do_translate(self, text: str) -> str:
               raise NotImplementedError
